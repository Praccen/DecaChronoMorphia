...
(function (MapGenerator) {
    async function GenerateMap(xSize, ySize, ecsManager, rendering) {
        ...
        for (let y = 1; y < labyrinth.length - 1; y += 2) {
            for (let x = 1; x < labyrinth[y].length - 1; x += 2) {
                if (labyrinth[x][y] == 0) {
                    const roomInformation = await createRoom(labyrinth, x, y, ecsManager, rendering);
        ...
        for (let i = -3; i < 4; i++) {
            for (let j = -2; j < 3; j++) {
                mapInformation.pointLightEntities.push(createPointLightEntity(new Vec3({ x: 0.15, y: 0.06, z: 0.08 }), new Vec3({ x: i, y: 1.5, z: j + 0.5 }), ecsManager, rendering));
            }
        }
    ...
    function createPointLightEntity(colour, pos, ecsManager, rendering) {
